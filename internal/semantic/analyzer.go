	...
	case *ast.PrintfStatement:
		...
		for _, arg := range s.Arguments {
			...
			if argType != ast.IntType {
				a.addError(arg.PositionInSource(), "printf argument must be int")
			}
		}
...
func (a *Analyzer) checkExpression(expr ast.Expression) ast.DataType {
	...
	case *ast.CallExpression:
		...
		if calleeType != ast.FunctionType {
			a.addError(e.PositionInSource(), "function call must have a function name")
			return -1
		}

		varExpr := e.Callee.(*ast.VariableExpression)
